...
namespace Phonedata
{
    ...
    public class PhoneData
    {
        ...
        public struct PhoneRecord
        {
            ...
            {
                string tmp = $"PhoneNum: {this.PhoneNum}\nAreaZon: {this.AreaZon}\nCardType: {this.CardType}\nCity: {this.City}\nZipCode: {this.ZipCode}\nProvince: {this.Province}\n";
                return tmp;
            }
        ...
        /// <summary>
        /// class phonedata 的构造函数 use EmbeddedResource 
        /// </summary>
        public PhoneData()
        {
            string phone_dat_path = "phone.dat";
            var assembly = GetType().Assembly;
            using (Stream stream = assembly.GetManifestResourceStream(phone_dat_path))
            {
        ...
        /// </summary>
        public PhoneData(string pdFilePath)
        {
            Init(pdFilePath);
        }
        ...
        /// </summary>
        /// <param name="b"></param>
        /// <returns>4 byte to int32 </returns>
        private int Get4(byte[] b)
        {
            int tmp;
            if (b.Length < 4)
            {
            ...
            tmp = (int)(b[0]) | ((int)(b[1])) << 8 | ((int)(b[2])) << 16 | ((int)(b[3])) << 24;
            return tmp;
        }
        ...
        private string GetCardType(byte b)
        {
            ...
            switch (type)
            {
                case (int)CardType.CMCC:
                    tmp = "中国移动";
                    break;
                case (int)CardType.CMCC_V:
                    tmp = "中国移动虚拟运营商";
                    break;
                case (int)CardType.CTCC:
                    tmp = "中国电信";
                    break;
                case (int)CardType.CTCC_V:
                    tmp = "中国电信虚拟运营商";
                    break;
                case (int)CardType.CUCC:
                    tmp = "中国联通";
                    break;
                case (int)CardType.CUCC_V:
                    tmp = "中国联通虚拟运营商";
                    break;
                default:
                    tmp = "未知电信运营商";
        ...
        /// 初始化程序
        /// </summary>
        /// <param name="pdFilePath"></param>
        public void Init(string pdFilePath)
        {
            if (!File.Exists(pdFilePath))
            {
                Trace.Assert(true, "PhoneData not Exists");
                return;
            }
            using (FileStream fs = new FileStream(pdFilePath, FileMode.OpenOrCreate, FileAccess.Read))
            {
                SetBufInfo(fs);
            }
        ...
        /// <returns></returns>
        public PhoneRecord Lookup(long phone)
        {
